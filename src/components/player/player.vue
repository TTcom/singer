	...
	export default{
		...
		methods:{
			      ...
			      middleTouchMove(e){
			        ...
			        this.$refs.lyriclist.$el.style[transform] =  `translate3d(${offsetWidth}px,0,0)`  //属于组件上的refs所以要加上$el
			      ...
			      getLyric(){
	                 ...
	                 this.timer1 = setTimeout(()=>{   //当数据或dom发生变化，更新后执行回调。避免数据更新后事件无法执行

                     let hhtime = this.$refs.audio.currentTime
			            this.currentLyric.play();
			           // this.currentLyric.seek(0)
			             this.currentLyric.seek(hhtime * 1000)
//			      this.currentLyric.play();
//			      this.currentLyric.seek(0)
//		          this.$refs.lyriclist.scrollTo(0,0) 
			      },1500)
...
//	                 this.timer2= setTimeout(()=>{   //当数据或dom发生变化，更新后执行回调。避免数据更新后事件无法执行
...
//			      	  },1000)
			...
			togglePlaying(){
				...
				if(!this.playing){
					...
					this.$refs.audio.play();
					this.currentLyric.togglePlay();  //togglePlay接着滚动
				}
		...
		watch:{
			currentSong(newSong,oldSong){  
	                 ...
	                 this.timer = setTimeout(()=>{   //当数据或dom发生变化，更新后执行回调。避免数据更新后事件无法执行
	                 	this.$refs.audio.play();
	                 	this.getLyric();
	                },200)    
			}
